minerva.views.TerraDiffInDiffWidget = minerva.View.extend({

    events: {
        'submit #m-terra-diff-in-diff-form': function (e) {
            e.preventDefault();
            this.$('.g-validation-failed-message').text('');
            this.$el.modal('hide');

            this.terraDiffInDiffPlot = new minerva.views.TerraDiffInDiffPlot({
                el: '.terraDiffInDiffPlot',
                model: new minerva.models.DDDatasetModel({
                    targetLocation: $('#m-terra-diff-in-diff-input-location').val(),
                    comparisonLocations: $('#m-terra-diff-in-diff-input-comparison-locations').val(),
                    eventDate: new Date($('.m-terra-diff-in-diff-input-dataset input[name="daterangepicker"]').val())
                }),
                parentView: this
            });
        }
    ...
    render: function () {
        ...
        var modal = this.$el.html(minerva.templates.terraDiffInDiffWidget({
            locations: locations.sort()
        })).girderModal(this).on('ready.girder.modal', _.bind(function () {
            $('#m-terra-diff-in-diff-form input[name="daterangepicker"]').daterangepicker({
